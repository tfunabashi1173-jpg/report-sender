...
<main class="page">
	...
	<section class="editor">
		...
		<form method="POST" action="?/save">
			...
			<textarea name="body" rows="8" required placeholder="本文を入力。{name} などの差し込み文字はそのまま残せます。"></textarea>
			<button>保存する</button>
		</form>
	</section>
			...
			{#each data.templates as template}
				<details class="template">
					...
					<form method="POST" action="?/save">
						...
						<label>
							本文
							<textarea name="body" rows="8" required>{template.body}</textarea>
						</label>
						<button>更新する</button>
					</form>
...
<style>
	...
	.templates { display: grid; gap: 12px; margin-top: 16px; }
